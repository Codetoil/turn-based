...
import io.github.codetoil.turnbased.events.TurningEntityEvent;
import io.github.codetoil.turnbased.world.LitTeleporter;
import io.github.codetoil.turnbased.world.LitWorldMethods;
...
public class TurningEntity {
	...
	public void teleportTo(int laterDimension) {
		if (!FMLCommonHandler.instance().getMinecraftServerInstance().getEntityWorld().isRemote) {
			try {
				...
				//if (oldWorld.getWorldType().getId() == 8748 && LitWorldMethods.containsEntity(this.uuid, oldWorld)) {
	...
	public boolean isPlayer() {
		return entity instanceof EntityPlayerSP || entity instanceof EntityPlayerMP || entity instanceof EntityOtherPlayerMP || entity instanceof EntityPlayer;
	}
}
